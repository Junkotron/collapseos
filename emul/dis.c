...
static void mk_sorted_file()
{
  char *buf[TOTAL_OP_ROWS];

  int i;
  for (i=0;i<TOTAL_OP_ROWS;i++)
    {
      buf[i]=(char*)malloc(100*sizeof(char));
    }
    

  FILE* lst = fopen("opcodes.lst", "r");
  ...
  qsort(buf, TOTAL_OP_ROWS, sizeof(char*), mystrcmp);

  FILE* lsts = fopen("opcodes.lst_sorted", "w");
  for (i=0;i<TOTAL_OP_ROWS;i++)
    {
      fprintf(lsts, "%s", buf[i]);
    }
...
void parse_ops(int be, int en)
{
  ...
  FILE* lst = fopen("libz80/codegen/opcodes.lst_sorted", "r");
  char buf[100];
